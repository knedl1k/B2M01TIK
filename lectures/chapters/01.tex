...
\begin{itemize}
	\item $ax = b$ v $\Z_n$
	\item $ax = b \pmod{n}$ v $\Z$
	\item $ax + ny = $ v $\Z$
\end{itemize}
\vspace{-1em}
Lineární rovnice $ax = b$ má řešení v $\Z_n$ právě, když $\gcd(a, n) |b$.

Je-li $x_p$ jedno řešení, pak každé řešení má tvar
\begin{equation}
	x = x_p + k \cdot x_0, \, \text{kde } x_0 = \frac{n}{\gcd(a,n)}, \, 0 \leq k < \gcd(a,n).
\end{equation}
...
\vspace{-1em}
\begin{itemize}
	\item $\varphi(n) = p-1$ pro $p$ \hyperref[prvocisla]{prvočíslo}
...
\subsection{Tvrzení o dělitelnosti řádu prvku}
